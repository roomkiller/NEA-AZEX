...
export default function PublicHealthMonitor() {
    return (
        <ProfessionalCenterTemplate
            domain="Santé_Publique"
            title="Surveillance Santé Publique"
            subtitle="Monitoring épidémiologique et alertes sanitaires"
            icon={Activity}
            iconColor="red"
            relatedDataConfig={async () => {
                const [predictions, signals, trends] = await Promise.all([
                    base44.entities.EventPrediction.filter({ event_type: 'SANITAIRE' }, '-probability_score', 5),
            ...
            statsConfig={(briefs, relatedData) => ({
                ...
                criticalThreats: { value: briefs.filter(b => b.priority_level === 'Critique' || b.priority_level === 'Flash').length, label: "Alertes Sanitaires" },
                predictions: { value: relatedData.predictions.length, label: "Prédictions" },
                custom: { value: Math.floor(Math.random() * 8) + 3, label: "Foyers Épidémiques" }
            })}
        />
    );
}